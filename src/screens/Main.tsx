        ...
        <Html fullscreen>
            ...
            <div className={`flex flex-col w-fit ${theme?.glassColor} bg-opacity-50 bg-clip-padding backdrop-filter backdrop-blur-md rounded-2xl shadow-xl ${theme?.glassShadowColor} lg:space-y-8 md:space-y-6 space-y-4 p-8 duration-200`}>
              ...
              <div className='flex space-x-4'>
                <button
                  ...
                  className={`rounded-full h-4 w-4 bg-indigo-900 ${theme?.themeName === 'Purple' ? 'ring-4 ring-neutral-900/25 scale-125' : null} duration-200`}>
                </button>
                <button
                  onClick={(): void => {setTheme(fetchedThemes[1])}}
                  className={`rounded-full h-4 w-4 bg-amber-200 ${theme?.themeName === 'Gold' ? 'ring-4 ring-neutral-900/25 scale-125' : null} duration-200`}>
                </button>
                <button
                  onClick={(): void => {setTheme(fetchedThemes[2])}}
                  className={`rounded-full h-4 w-4 bg-slate-200  ${theme?.themeName === 'Silver' ? 'ring-4 ring-neutral-900/25 scale-125' : null} duration-200`}>
                </button>
                <button
                  onClick={(): void => {setTheme(fetchedThemes[3])}}
                  className={`rounded-full h-4 w-4 bg-neutral-900 ${theme?.themeName === 'Black' ? 'ring-4 ring-neutral-900/25 scale-125' : null} duration-200`}>
                </button>
              </div>
          ...
          <div className='absolute bottom-8 right-8'>
            <div className='flex flex-col space-y-8'>
              <button><IoLogoDribbble className='lg:text-2xl md:text-xl sm:text-lg hover:scale-125 duration-200' /></button>
              <button><IoLogoInstagram className='lg:text-2xl md:text-xl sm:text-lg hover:scale-125 duration-200' /></button>
              <button><IoLogoLinkedin className='lg:text-2xl md:text-xl sm:text-lg hover:scale-125 duration-200' /></button>
            </div>
          </div>
        </Html>
        {/* Initializes cameras */}
